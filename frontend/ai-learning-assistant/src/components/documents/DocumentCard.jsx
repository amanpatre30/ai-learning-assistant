...
const DocumentCard = ({ document, onDelete }) => {
 ...
 return (
  <div className="group relavtive bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl p-5 hover:border-slate-300/60 hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 flex flex-col justify-between cursor-pointer hover:-translate-y-1" onClick={handleNavigate}>
   {/** Header Section */}
   <div>
    <div className="flex items-start justify-between gap-3 bg-4">
    ...
    {/** Title */}
    <h3 className="text-base mt-2 font-semibold text-slate-900 truncate mb-2" title={document.title}>
     {document.title}
    </h3>
   ...
   {/** Hover Indicator */}
   <div className="absolute inset-0 rounded-2xl bg-linear-to-br from-emerald-500/0 to-teal-500/0 group-hover:from-emerald-500/5 group-hover:to-teal-500/5 transition-all duration-300 pointer-300 pointer-events-none" />
  </div>
 );
};
